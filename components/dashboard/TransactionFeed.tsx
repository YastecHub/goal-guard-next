  ...
  return (
    <div>
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
        {/* Table header */}
        <div className="grid grid-cols-[auto_1fr_auto_auto] gap-4 px-5 py-3 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200/80 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider">
          <span>Type</span>
        ...
        {transactions.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
            {transactions.slice(0, 15).map((tx) => {
              ...
              return (
                <div key={tx.id} className="grid grid-cols-[auto_1fr_auto_auto] gap-4 px-5 py-3.5 items-center hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                  {/* Type icon */}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-xs font-medium">{tx.date} · {tx.category}</p>
                  </div>

                  {/* Amount */}
                  <p className={`text-sm font-extrabold tabular-nums whitespace-nowrap ${isCredit ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>
                    {isCredit ? '+' : '-'}₦{tx.amount.toLocaleString()}
                  </p>

                  {/* Status badge */}
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-lg whitespace-nowrap ${STATUS_STYLES[tx.status]}`}>
                    {tx.status}
                  </span>
                </div>
              );
